...
When (/^I open calendar and click on calendar shortcut$/) do
  ...
  @pages.main.count_calendar_shortcuts.times do |index|
    begin
      ...
      expected_date_from, expected_date_until = @pages.main.get_calendar_expected_dates_by_shortcut_text(shortcut_button.get_text_when_appears)

      shortcut_button.click

      @pages.main.validate_calendar_dates(expected_date_from, expected_date_until)
    rescue => exception
